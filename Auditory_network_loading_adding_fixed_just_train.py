...
# modelname here is the name of the pretrained model
network = SpikingNetwork(n_neurons=400, inpt_shape=(1, data_dim), n_inpt=data_dim, dt=dt,
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        # Get next input sample
        inputs_L2 = {"Input": spikes1["Excitatory"].get("s").squeeze().view(int(pattern_time / dt), 1, 1, n_neurons)}
        if gpu:
            inputs_L2 = {k: v.cuda() for k, v in inputs_L2.items()}

        network2.run(inputs=inputs_L2, time=pattern_time, input_time_dim=1)
        ...
        if (step >= (n_train - 1)) or (step % update_interval == 0 and step > 0):
            ...
            #L2
            input_exc_weights_L1 = network.connections[("Input", "Excitatory")].w
            square_weights_L1 = get_square_weights(input_exc_weights_L1.view(data_dim, n_neurons_L2), data_dim_sqrt,
                                                data_dim_sqrt)
            input_exc_weights_L2 = network2.connections[("Input", "Excitatory")].w
            ...
            plot_weights(square_weights_L1, im=weights_im,
