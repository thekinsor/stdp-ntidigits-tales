...
from population_eval import assign_pop_labels, pop_avg_prediction, pop_decorrelated_prediction, pop_diff_prediction, pop_prediction
...
#load the label profiles
label_profiles = torch.load("results/" + filename + "/label_profiles/final_profiles.pt")
# label_profiles = torch.load("results/" + filename + "/whitened_profiles_from_final_profiles_36_percent.pt")
# label_profiles = (label_profiles).to(device)
#label_profiles = torch.nn.functional.normalize(label_profiles,dim=0,p=2)

#get zca
# zca_matrix = torch.load("results/" + filename + "/zca_matrix_from_final_profiles_36_percent.pt")
# zca_matrix = (zca_matrix).to(device)

#print(label_profiles.sum(dim=0))

#create a thresholded version with threshold 0.05 and normalize again
thresholded_label_profiles = torch.where(label_profiles>=0.005, label_profiles, torch.zeros_like(label_profiles))
thresholded_label_profiles = thresholded_label_profiles/thresholded_label_profiles.sum(dim=0,keepdim=True)
...
# Sequence of accuracy estimates
accuracy = {"pop": 0, "avg-diff": 0, "diff-abs": 0, "thresh": 0}

# Record spikes during the simulation
spike_record = torch.zeros((1, int(pattern_time / dt), n_neurons), device=device)
testing_vectormult_pred = torch.zeros(n_test, dtype=torch.int64, device=device)
testing_vectormult_label_tensor = torch.zeros(n_test, dtype=torch.int64, device=device)
...
testing_thresh_pred = torch.zeros(n_test, dtype=torch.int64, device=device)
testing_thresh_label_tensor = torch.zeros(n_test, dtype=torch.int64, device=device)
...
for step, batch in enumerate(test_dataset):
    ...
    # Get network predictions for pop
    ...
    testing_vectormult_pred[step] = pop_activity_pred
    testing_vectormult_label_tensor[step] = label_tensor
    ...
    pop_activity_pred = pop_prediction(
        spikes=spike_record, label_profiles=thresholded_label_profiles, n_labels=n_classes,
    )

    # Compute network accuracy according to available classification strategies
    accuracy["thresh"] += float(torch.sum(label_tensor.long() == pop_activity_pred).item())
    
    testing_thresh_pred[step] = pop_activity_pred
    testing_thresh_label_tensor[step] = label_tensor

    network.reset_state_variables()  # Reset state variables
    pbar.set_description_str("Test progress: ")
    pbar.update()

plot_confusion_matrix(torch.Tensor.cpu(testing_vectormult_pred), torch.Tensor.cpu(testing_vectormult_label_tensor),
                      save=f'./results/{filename}/test_confusion_matrix_vectormult_{filename}.png')
...
plot_confusion_matrix(torch.Tensor.cpu(testing_thresh_pred), torch.Tensor.cpu(testing_thresh_label_tensor),
                      save=f'./results/{filename}/test_confusion_matrix_thresh_0_5_{filename}.png')

print("\nPopulation scalar product accuracy: %.2f" % (accuracy["pop"] / n_test))
...
print("\nPopulation averaged out accuracy: %.2f" % (accuracy["avg-diff"] / n_test))

print("Progress: %d / %d (%.4f seconds)" % (1, n_epochs, t() - start))
print("Testing complete.\n")

# Save logs.
with open(f'./results/{filename}/log_{filename}_four_schemes.txt', 'w+') as log_file:
    print(f'''
    Pop activity accuracy final: {accuracy["pop"] / n_test}
    Population threshholded accuracy: {accuracy["thresh"] / n_test} \n
    Population difference based accuracy: {accuracy["diff-abs"] / n_test} \n
    Population averaged out accuracy: {accuracy["avg-diff"] / n_test} \n
    Progress: {1} / {n_epochs} ({t() - start} seconds)
    Testing complete.\n
