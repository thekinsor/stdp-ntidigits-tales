...
parser = argparse.ArgumentParser(description='Train a SNN on the NMNIST dataset.')
# Neuron parameters
...
# Network parameters
parser.add_argument("--n_neurons", type=int, default=400, help='Number of neurons in the excitatory layer.')
...
# Defaults
parser.set_defaults(plot=False, gpu=True, som=False)
...
# Load training data
train_dataset = NMNIST(root=os.path.join("./", "data"), download=True, train=True, dt=dt, transform=SparseToDense())
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        if (step >= (n_train - 1)) or (step % update_interval == 0 and step > 0):
            cumulative_spikes[(step - 1) // update_interval, :].add_(torch.sum(spike_record.long(), (0, 1)))
            input_exc_weights = network.connections[("Input", "Excitatory")].w
            square_weights = get_square_weights(input_exc_weights.view(data_dim_sq, n_neurons), n_neurons_sqrt,
                                                data_dim)
            square_assignments = get_square_assignments(assignments, n_neurons_sqrt)
...
# Load testing data
test_dataset = NMNIST(root=os.path.join("./", "data"), download=True, train=False, dt=dt, transform=SparseToDense())
