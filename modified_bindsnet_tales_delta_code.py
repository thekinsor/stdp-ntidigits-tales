...
from custom_modules_tales import DeltaNeurons, FactoredDelayedConnection, DiehlAndCookNodesContinual, DiehlCookDelayedSTDP, JustPassOnConnection, PostPreDelayed
...
class SpikingNetwork(Network):
    ...
    def __init__(
        ...
        #on and off layers before exc for delta code
        on_cells = DeltaNeurons(
            n=self.n_inpt, shape=self.inpt_shape, traces=True, tc_trace=tc_trace, traces_additive=traces_additive, on_cells=True, burst_time=burst_time,
        )
        off_cells = DeltaNeurons(
            n=self.n_inpt, shape=self.inpt_shape, traces=True, tc_trace=tc_trace, traces_additive=traces_additive, on_cells=False, burst_time=burst_time,
        )
        ...
        #create connections from input to on and off populations
        w_on_off = torch.diag(torch.ones(self.n_inpt))
        inp_on_conn = JustPassOnConnection(source=input_layer, target=on_cells, w=w_on_off)
        inp_off_conn = JustPassOnConnection(source=input_layer, target=off_cells, w=w_on_off)

        # Connections
        w_on = winit * torch.rand(self.n_inpt, self.n_neurons)
        w_off = winit * torch.rand(self.n_inpt, self.n_neurons)
        ...
        if(delayed):
            ...
            #random delays
            d_on = torch.randint(low=dmin, high=dmax, size=(self.n_inpt, self.n_neurons))
            d_off = torch.randint(low=dmin, high=dmax, size=(self.n_inpt, self.n_neurons))

            #load the delay profiles obtained from the delta code offline
            #d_on = torch.load("results/d_on_prototypes.pt")
            #d_off = torch.load("results/d_off_prototypes.pt")
            ...
            on_exc_conn = FactoredDelayedConnection(
                source=on_cells,
                target=exc_layer,
                w=w_on,
            ...
            on_exc_conn.update_rule.x_tar = x_tar  # PARAMETER

            off_exc_conn = FactoredDelayedConnection(
                source=off_cells,
                target=exc_layer,
                w=w_off,
                update_rule=DiehlCookDelayedSTDP,  # FUTURE WORK: Other learning rules could also be explored.
            ...
            off_exc_conn.update_rule.x_tar = x_tar  # PARAMETER
        else:
            on_exc_conn = FactoredConnection(
                source=on_cells,
                target=exc_layer,
                w=w_on,
            ...
            off_exc_conn = FactoredConnection(
                source=off_cells,
                target=exc_layer,
                w=w_off,
                update_rule=DiehlCookSTDP,  # FUTURE WORK: Other learning rules could also be explored.
        ...
        if self.recurrency:
            if(delayed):
                ...
                exc_exc_conn.update_rule.x_tar = x_tar  # PARAMETER
            else:
                w = winit * torch.rand(self.n_neurons, self.n_neurons)
        ...
        self.add_layer(on_cells, "ONCells")
        self.add_layer(off_cells, "OFFCells")
        self.add_layer(exc_layer, name="Excitatory")
        self.add_layer(inh_layer, name="Inhibitory")
        self.add_connection(inp_on_conn, source="Input", target="ONCells")
        self.add_connection(inp_off_conn, source="Input", target="OFFCells")
        self.add_connection(on_exc_conn, source="ONCells", target="Excitatory")
        self.add_connection(off_exc_conn, source="OFFCells", target="Excitatory")
